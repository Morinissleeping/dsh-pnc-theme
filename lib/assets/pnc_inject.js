(function () {
  ...
  // v158：速度类参数 ms 直显
  // v0.2.17：默认值 = 作者最舒服的配置（淡毛玻璃 0.25 / 面板 0.9 / 康威 0.55 密度 1.6）
  ...
  // v0.2.14：matrix 加载指示显示/隐藏（配额条左侧）
  function setQuotaLoading(on) {
    var m = document.querySelector('.pnc-quota-matrix');
  ...
  function updateQuota(instant) {
    try {
      ...
      // v0.2.14：开始获取 → 显示 matrix；数据应用后等进度条 transition（0.6s）结束再隐藏
      ...
      fetch('/pnc-quota-data.json').then(function (r) { return r.json(); }).then(function (q) {
        ...
        setTimeout(function () { setQuotaLoading(false); }, instant ? 100 : 750);      }).catch(function () { setTimeout(function () { setQuotaLoading(false); }, 150); });
    } catch (e) {}
  }
  // v0.2.14：暴露刷新句柄（外部/调试可触发配额刷新）
  window.__pncUpdateQuota = updateQuota;
  function buildQuota() {
    var q = document.createElement('div');
    q.className = 'pnc-quota';
    // v0.2.14/0.2.16：左侧 matrix 加载指示——复用 DSH _matrix_10orb_4 组件结构
    // （外圈 8 个 2x2 cell + dot-chase 动画 + 负 delay 错开 125ms = 与命令块运行中一致的追逐加载效果）
    var mx = document.createElement('div');
    mx.className = 'pnc-quota-matrix';
    mx.setAttribute('aria-hidden', 'true');
    var z0 = [[0, 0], [4, 0], [8, 0], [8, 4], [8, 8], [4, 8], [0, 8], [0, 4]];
    var svg = '<svg class="_matrix_10orb_4" data-state="ongoing" width="10" height="10" viewBox="0 0 10 10" shape-rendering="crispEdges" aria-hidden="true">' +
      z0.map(function (p, c) {
        return '<rect class="_cell_10orb_54" x="' + p[0] + '" y="' + p[1] + '" width="2" height="2" style="animation-delay:' + ((c - z0.length) * 125) + 'ms"></rect>';
      }).join('') + '</svg>';
    mx.innerHTML = svg;
    q.appendChild(mx);
