...
- **背景**（视频/图片合并入口）：上传 mp4/webm 或 PNG/JPEG/GIF/WebP（图片存在时优先于视频）、一键重置回包内默认视频、背景不透明度（视频与图片通用）；上传/重置后页面自动刷新生效
- **视觉主题**：
...
dsh plugin --profile web add https://github.com/Morinissleeping/dsh-pnc-theme/releases/download/v0.2.3/dsh-external-dsh-pnc-theme-0.2.3.tgz
