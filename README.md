...
- **毛玻璃层**：独立模糊层 `#pnc-bg-glass`——层序：视频/图片(z:-3) < 毛玻璃层(z:-2) < 所有其他元素；仅覆盖**中央聊天区**（上栏/侧栏区域背景保持清晰），backdrop-filter blur(20px) + saturate(150%)；「毛玻璃不透明度」滑条控制层浓度（默认 90%），「面板透度」滑条控制面板底色透明度（默认 55%）
